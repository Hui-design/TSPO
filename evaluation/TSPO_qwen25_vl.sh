...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch --num_processes 8 --main_process_port 12345 -m lmms_eval \
    ...
    --model_args pretrained=/your/path/lmms-lab/LLaVA-Video-7B-Qwen2,is_uniform=False,device_map=auto,max_num_frames=64,fps=1,max_pixels=235200,attn_implementation="flash_attention_2" \
    --tasks "${task_name}" \
    --batch_size 1 \
    --log_samples \
