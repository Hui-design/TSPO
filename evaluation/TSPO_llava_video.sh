...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch --num_processes 8 --main_process_port 12345 -m lmms_eval \
    ...
    --model_args pretrained=/your/path/lmms-lab/LLaVA-Video-7B-Qwen2,is_uniform=False,conv_template=chatml_direct,video_decode_backend=decord,max_frames_num=64,overwrite=False \
    --tasks "${task_name}" \
    --batch_size 1 \
    --log_samples \
